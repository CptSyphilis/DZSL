...
class ServersView:
    ...
    def _update_map_filter(self):
        ...
        prev_id = ""
        if _filter_state.get("map_ids") and _filter_state.get("map", 0) > 0:
            idx = min(_filter_state["map"], len(_filter_state["map_ids"]) - 1)
            prev_id = _filter_state["map_ids"][idx]
        ...
        if hasattr(self, "f_map"):
            ...
            if prev_id and prev_id in ids:
                self.f_map.set_selected(ids.index(prev_id))
            else:
                self.f_map.set_selected(min(_filter_state.get("map", 0), len(labels) - 1))
